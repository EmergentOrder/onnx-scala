 ...
 * arithmetic, then convert back to a BFloat16 if needed.
 *
 ...
 *
 *     0 00000000 0000000  (positive) zero
 *     1 00000000 0000000  negative zero
 *     _ 00000000 _______  subnormal number
 *     _ 11111111 0000000  +/- infinity
 *     _ 11111111 _______  not-a-number
 *     _ ________ _______  normal number
 *
 * An exponent of all 1s signals a sentinel (NaN or infinity), and all
 * 0s signals a subnormal number. So the working "real" range of
 * exponents we can express is [-126, +127].
 *
 * For non-zero exponents, the mantissa has an implied leading 1 bit,
 * so 7 bits of data provide 8 bits of precision for normal numbers.
 *
 * For normal numbers:
 *
 *     x = (1 - sign*2) * 2^exponent * (1 + mantissa/128)
 *
 * For subnormal numbers, the implied leading 1 bit is absent. Thus,
 * subnormal numbers have the same exponent as the smallest normal
 * numbers, but without an implied 1 bit.
 *
...
class BFloat16(val raw: Short) extends AnyVal { lhs =>
   ...
   *
   *  * NaN: the value is BFloat16.NaN (and has no sign)
   *  * -1F: the value is a non-zero negative number
   *  * -0F: the value is BFloat16.NegativeZero
   *  *  0F: the value is BFloat16.Zero
...
object BFloat16 {
   ...
   *
   * This value is guaranteed to be the closest possible BFloat16
   ...
   * values which are not precisely representable as BFloat16 values.
   *
   */
