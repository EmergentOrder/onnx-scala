...
/** Type-class used to materialize the singleton type of a [[Shape]].
  *
  * This is useful to implicitly convert a type-level representation of a [[Shape]] to a term
  * representing the same [[Shape]], for instance by using the [[shapeOf]] method:
  *
  * {{{
...
object ShapeOf {
   ...
   given shapeOfCons[H <: Dimension, T <: Shape](using
       head: ValueOf[H],
       tail: ShapeOf[T]
   ): ShapeOf[H #: T] =
