...
//      inputs: Tuple,
   ...
   protected def createInputValueInfoProto[T, Ax <: Axes](
       tens: Tensor[T, Ax],
       inputName: String
   ): ValueInfoProto = {
               ...
               onnx.onnx.TypeProto.Tensor(
                 shape = Some(
                   onnx.onnx.TensorShapeProto(dim =
                      shape(tens).map(onnx.onnx.TensorShapeProto.Dimension().withDimValue(_))
                   )
                 ),
...
//              tup(0) match {
               case opt: Option[Tensor[T, _]] =>
                  opt match {
                     case Some(in) => {
                        Some((createInputValueInfoProto(in, i.toString), i.toString))
                     }
                     case None => None
                  }
               case tens: Tensor[T, _] => {
                  Some((createInputValueInfoProto(tens, i.toString), i.toString))
               }
            }
//          }
         }.flatten
      ...
      val node = nodeWithAddedInputs(
        ...
        opToNode(name, opName, outName, attrs),
        name
      )
      ...
      val model = ModelProto(
        ...
        graph = Some(newGraph),
        irVersion = Some(6),
        opsetImport = Seq(OperatorSetIdProto(version = Some(12)))
      )
