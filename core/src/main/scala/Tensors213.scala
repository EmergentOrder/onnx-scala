...
//  type Dimension = Int with Singleton
   ...
   type TensorRank4Shape[I <: Dimension, J <: Dimension, K <: Dimension, L <: Dimension] =
      (I, J, K, L) with Shape
   ...
   sealed case class Vec[T <: TensorTypeDenotation, D <: DimensionDenotation, Q <: Shape](
       i: Dimension
   ) extends Axes
   sealed case class Mat[
       T <: TensorTypeDenotation,
       D <: DimensionDenotation,
       D1 <: DimensionDenotation,
       Q <: Shape
   ](i: Dimension, j: Dimension)
       extends Axes

   sealed case class TensorRank3[
       T <: TensorTypeDenotation,
       D <: DimensionDenotation,
       D1 <: DimensionDenotation,
       D2 <: DimensionDenotation,
       Q <: Shape
   ](
       i: Dimension,
       j: Dimension,
   ...
   sealed case class TensorRank4[
       T <: TensorTypeDenotation,
       D <: DimensionDenotation,
       D1 <: DimensionDenotation,
       D2 <: DimensionDenotation,
       D3 <: DimensionDenotation,
       Q <: Shape
   ](
       i: Dimension,
       j: Dimension,
   ...
   type Tensor[T, A <: Axes] = OSTensor[T, A] //(Array[T], Array[Int])

   type SparseTensor[T, A <: Axes] = Tensor[T, A]
...
//
//TODO: opaque
   type OSTensor[T, A <: Axes] = Tuple2[Array[T], A]

   object Tensor {
      def data[T, A <: Axes](tens: OSTensor[T, A]) = tens._1
//    def apply[T <: Supported] (elem: T): OSTensor[T, Scalar] = new OSTensor[T, ?](Array(elem), Scalar())
      def shape[T, A <: Axes](tens: OSTensor[T, A]) = tens._2 match {
      ...
      def tensorRequires[T, A <: Axes](tens: OSTensor[T, A]): OSTensor[T, A] = {
      ...
      def apply[T, Tt <: TensorTypeDenotation, D <: DimensionDenotation](
          arr: Array[T]
      ): OSTensor[T, Scalar[Tt, D]] = tensorRequires(arr, Scalar[Tt, D]())

      def apply[T, Tt <: TensorTypeDenotation, D <: DimensionDenotation](
          arr: Array[T],
          d0: Dimension
      ): OSTensor[T, Vec[Tt, D, VecShape[d0.type]]] =
         tensorRequires(arr, Vec[Tt, D, VecShape[d0.type]](d0))
      def apply[T, Tt <: TensorTypeDenotation, D <: DimensionDenotation, D1 <: DimensionDenotation](
          arr: Array[T],
          d0: Dimension,
          d1: Dimension
      ): OSTensor[T, Mat[Tt, D, D1, MatShape[d0.type, d1.type]]] =
         (arr, Mat[Tt, D, D1, MatShape[d0.type, d1.type]](d0, d1))
      def apply[
          T,
          ...
          d1: Dimension,
          d2: Dimension
      ): OSTensor[T, TensorRank3[Tt, D, D1, D2, TensorRank3Shape[d0.type, d1.type, d2.type]]] =
         (arr, TensorRank3[Tt, D, D1, D2, TensorRank3Shape[d0.type, d1.type, d2.type]](d0, d1, d2))
      ...
      def apply[
          ...
          D2 <: DimensionDenotation,
          D3 <: DimensionDenotation
      ](arr: Array[T], d0: Dimension, d1: Dimension, d2: Dimension, d3: Dimension): OSTensor[
        T,
        TensorRank4[Tt, D, D1, D2, D3, TensorRank4Shape[d0.type, d1.type, d2.type, d3.type]]
      ] = (
        arr,
        TensorRank4[Tt, D, D1, D2, D3, TensorRank4Shape[d0.type, d1.type, d2.type, d3.type]](
          d0,
      ...
      def create[T, Ax <: Axes](arr: Array[T], shape: Array[Int]): OSTensor[T, Ax] = {
         shape.size match {
            case 0 => apply(arr).asInstanceOf[OSTensor[T, Ax]]
            case 1 => apply(arr, shape(0).asInstanceOf[Dimension]).asInstanceOf[OSTensor[T, Ax]]
            case 2 =>
               apply(arr, shape(0).asInstanceOf[Dimension], shape(1).asInstanceOf[Dimension])
                  .asInstanceOf[OSTensor[T, Ax]]
            case 3 =>
               apply(
                 arr,
                 shape(0).asInstanceOf[Dimension],
                 shape(1).asInstanceOf[Dimension],
                 shape(2).asInstanceOf[Dimension]
               ).asInstanceOf[OSTensor[T, Ax]]
            case 4 =>
               apply(
                 arr,
                 ...
                 shape(1).asInstanceOf[Dimension],
                 shape(2).asInstanceOf[Dimension],
                 shape(3).asInstanceOf[Dimension]
               ).asInstanceOf[OSTensor[T, Ax]]
            case _ => ???
         }
      }
