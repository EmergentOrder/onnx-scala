...
trait NCF {
  ...
  def program(
    ...
    for {
      ...
      nodeaffine_outputweight <- List(
        dataSource.getParams[Float]("affine_output.weight")
      )
      nodefc_layers0bias <- List(
        dataSource.getParams[Float]("fc_layers.0.bias")
      )
      nodefc_layers0weight <- List(
        dataSource.getParams[Float]("fc_layers.0.weight")
      )
      nodefc_layers1bias <- List(
        dataSource.getParams[Float]("fc_layers.1.bias")
      )
      nodefc_layers1weight <- List(
        dataSource.getParams[Float]("fc_layers.1.weight")
      )
      nodefc_layers2bias <- List(
        dataSource.getParams[Float]("fc_layers.2.bias")
      )
      nodefc_layers2weight <- List(
        dataSource.getParams[Float]("fc_layers.2.weight")
      )
