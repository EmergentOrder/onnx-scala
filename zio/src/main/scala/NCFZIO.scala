...
val onnxHelper = new ONNXHelper("NCF.onnx")
  ...
  def program(inputDataactual_input_1: Task[Tensor[Long]],inputDatalearned_0: Task[Tensor[Long]]): Task[Tensor[Float]]  = 
    for {
      ...
      nodefc_layers0weight <-  dataSource.getParamsZIO[Float]("fc_layers.0.weight")
      nodefc_layers1bias <-  dataSource.getParamsZIO[Float]("fc_layers.1.bias")
      nodefc_layers1weight <-  dataSource.getParamsZIO[Float]("fc_layers.1.weight")
      nodefc_layers2bias <-  dataSource.getParamsZIO[Float]("fc_layers.2.bias")
      nodefc_layers2weight <-  dataSource.getParamsZIO[Float]("fc_layers.2.weight")
      ...
      node14 <- GatherZIO.Gather1ZIO("14" ,data = Some(nodelearned_1),indices = Some(nodeactual_input_1))
      node15 <- GatherZIO.Gather1ZIO("15" ,data = Some(nodelearned_2),indices = Some(nodelearned_0))
      node16 <- GatherZIO.Gather1ZIO("16" ,data = Some(nodelearned_3),indices = Some(nodeactual_input_1))
      node17 <- GatherZIO.Gather1ZIO("17" ,data = Some(nodelearned_4),indices = Some(nodelearned_0))
      node18 <- ConcatZIO.Concat4ZIO("18" ,axis = Some((-1)),inputs = Seq(Some(node14),Some(node15)))
      node19 <- MulZIO.Mul7ZIO("19" ,A = Some(node16),B = Some(node17))
      node20 <- GemmZIO.Gemm9ZIO("20" ,transB = Some((1)),A = Some(node18),B = Some(nodefc_layers0weight),C = Some(nodefc_layers0bias))
      node21 <- ReluZIO.Relu6ZIO("21" ,X = Some(node20))
      node22 <- GemmZIO.Gemm9ZIO("22" ,transB = Some((1)),A = Some(node21),B = Some(nodefc_layers1weight),C = Some(nodefc_layers1bias))
      node23 <- ReluZIO.Relu6ZIO("23" ,X = Some(node22))
      node24 <- GemmZIO.Gemm9ZIO("24" ,transB = Some((1)),A = Some(node23),B = Some(nodefc_layers2weight),C = Some(nodefc_layers2bias))
      node25 <- ReluZIO.Relu6ZIO("25" ,X = Some(node24))
      node26 <- ConcatZIO.Concat4ZIO("26" ,axis = Some((-1)),inputs = Seq(Some(node25),Some(node19)))
      node27 <- GemmZIO.Gemm9ZIO("27" ,transB = Some((1)),A = Some(node26),B = Some(nodeaffine_outputweight),C = Some(nodeaffine_outputbias))
